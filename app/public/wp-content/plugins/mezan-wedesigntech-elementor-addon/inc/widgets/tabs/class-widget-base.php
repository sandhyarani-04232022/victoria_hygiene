	...
	function __construct() {
		...
		// Options
			...
			$options['default'] = array(
				'image_or_icon' => esc_html__( 'Image or Icon', 'wdt-elementor-addon'),
		...
		// Module defaults
			$option_defaults = array(
				array(
					...
					'media_icon_style' => 'block',
					'item_title' => esc_html__( 'Ut accumsan mass', 'wdt-elementor-addon' ),
					'item_sub_title' => esc_html__( 'Pellentesque ornare', 'wdt-elementor-addon' ),
					...
					'item_template_title' => esc_html__( 'Vestibulum et augue', 'wdt-elementor-addon' )
				),
				array(
					'item_type' => 'default',
                    'media_type' => 'icon',
					...
					'item_template_title' => esc_html__( 'Suscipit pellentesque imperdiet', 'wdt-elementor-addon' )
				)
			);
								...
								'styles' => array (
									'color' => array (
										...
										'selector' => array (
											...
											'{{WRAPPER}} .wdt-tabs-container[class*="-template-bordered"] .wdt-tabs-list li .ui-tabs-anchor::before, {{WRAPPER}} .wdt-tabs-container[class*="-template-bordered"] .wdt-tabs-list li .ui-tabs-anchor::after, {{WRAPPER}} .wdt-tabs-container[class*="-template-classic"] .wdt-tabs-list li .ui-tabs-anchor:before, {{WRAPPER}} .wdt-tabs-container[class*="-tabs-toggle-mode"] .wdt-tabs-list-wrapper .wdt-tabs-list:before, {{WRAPPER}} .wdt-tabs-container[class*="-tabs-toggle-mode"] .wdt-tabs-list-wrapper .wdt-tabs-list:after' => 'background-color: {{VALUE}};',
							...
							$output .= '<li><a href="#wdt-tabs-'.esc_attr($key).'">';
								if( $item['item_type'] == 'template' ) {
									if($settings['template'] == 'hunch-back-icon') {
										...
										if($item['media_type_template'] == 'icon' && $settings['icon_show'] == 'true') {
											$output .= $this->cc_repeater_contents->render_template_icon($key, $item, $widget_object);
										} else if($item['media_type_template'] == 'image') {
                                            $output .= $this->cc_repeater_contents->render_template_image($item, '', '');
                                        }
										$output .= '<div class="wdt-content-title">'.$item['item_template_title'].'</div>';
										if( isset($item['item_sub_title']) && !empty($item['item_sub_title']) ) {
									...
									if($settings['template'] == 'hunch-back-icon') {
										...
										if($item['media_type'] == 'icon' && $settings['icon_show'] == 'true') {
											$output .= $this->cc_repeater_contents->render_icon($key, $item, $widget_object);
