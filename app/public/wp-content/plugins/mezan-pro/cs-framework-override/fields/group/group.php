...
class CSFramework_Option_group extends CSFramework_Options {
  ...
  public function output() {
    ...
    $acc_title   = ( isset( $this->field['accordion_title'] ) ) ? $this->field['accordion_title'] : esc_html__( 'Adding', 'mezan-pro' );
    ...
    $mezan_default_type    = ( isset( $this->field['mezan_default_type'] ) && !empty( $this->field['mezan_default_type'] ) ) ? true : false;
    ...
    echo '<div class="cs-group cs-group-'. $el_class .'-adding hidden">';
      ...
      if( !$mezan_default_type ) {
        echo '<div class="cs-element cs-text-right cs-remove"><a href="#" class="button cs-warning-primary cs-remove-group">'. esc_html__( 'Remove', 'mezan-pro' ) .'</a></div>';
      }
      echo '</div>';
