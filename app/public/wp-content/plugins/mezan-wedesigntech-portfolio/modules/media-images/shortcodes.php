	...
	function wdt_sp_media_images( $attrs, $content = null ) {
		...
		if($attrs['listing_id'] != '') {
			...
			$output .= '<div class="wdt-listings-image-gallery-holder '.esc_attr( $attrs['class'] ).' '.esc_attr( $add_class ).'">';
				...
				// Gallery Images
				$output .= '<div class="wdt-listings-image-gallery-container swiper-container wdt-media-carousel-module-id-'.esc_attr($attrs['module_id']).'" '.$media_carousel_attributes_string.'>';
