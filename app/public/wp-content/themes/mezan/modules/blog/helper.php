...
if( !function_exists('mezan_archive_blog_post_defaults') ) {
    function mezan_archive_blog_post_defaults() {
        $defaults = array(
...
if( !function_exists('mezan_archive_blog_post_misc_defaults') ) {
    function mezan_archive_blog_post_misc_defaults() {
        $defaults = array(
...
if( !function_exists('mezan_archive_blog_post_params_default') ) {
    function mezan_archive_blog_post_params_default() {
        $params = array(
            ...
            'archive_readmore_text'  => esc_html__('Read More', 'mezan'),
...
if( !function_exists('mezan_archive_blog_post_defaults_filter') ) {
    function mezan_archive_blog_post_defaults_filter() {
        $defaults = mezan_archive_blog_post_defaults();
        return apply_filters( 'mezan_archive_post_cmb_class', $defaults );
    }
}

if( !function_exists('mezan_archive_blog_post_misc_defaults_filter') ) {
    function mezan_archive_blog_post_misc_defaults_filter() {
        $defaults = mezan_archive_blog_post_misc_defaults();
        return apply_filters( 'mezan_archive_post_hld_class', $defaults );
    }
}

if( !function_exists('mezan_archive_blog_post_params') ) {
    function mezan_archive_blog_post_params() {
        $params = mezan_archive_blog_post_params_default();
        return apply_filters( 'mezan_archive_blog_post_params', $params );
    }
}

if( !function_exists('mezan_get_archive_post_combine_class') ) {
	function mezan_get_archive_post_combine_class() {

        $blog_defaults = mezan_archive_blog_post_defaults_filter();
        ...
        return apply_filters( 'mezan_get_archive_post_combine_class', implode( ' ', $combine_class ) );
	}
}

if( !function_exists('mezan_get_archive_post_holder_class') ) {
	function mezan_get_archive_post_holder_class() {

        $blog_defaults = mezan_archive_blog_post_defaults_filter();
        $blog_misc_defaults = mezan_archive_blog_post_misc_defaults_filter();
        ...
        return apply_filters( 'mezan_get_archive_post_holder_class', implode( ' ', $holder_class ) );
	}
}

if( !function_exists('mezan_get_archive_post_style') ) {
	function mezan_get_archive_post_style() {
		...
		return apply_filters( 'mezan_get_archive_post_style', $post_style );
	}
}

if( !function_exists('mezan_get_archive_post_column') ) {
	function mezan_get_archive_post_column() {
		...
		return apply_filters( 'mezan_get_archive_post_column', $post_columns );
	}
}

add_filter('post_class', 'mezan_archive_set_post_class', 10, 3);
if( !function_exists('mezan_archive_set_post_class') ) {
    function mezan_archive_set_post_class( $classes, $class, $post_id ) {
        ...
        if( is_post_type_archive('post') || is_search() || is_category() || is_tag() || is_home() || is_author() || is_year() || is_month() || is_day() || is_time() || is_tax('post_format') || ( defined('DOING_AJAX') && DOING_AJAX ) ) {
            ...
            $blog_params = mezan_archive_blog_post_params();
...
add_action( 'mezan_after_main_css', 'mezan_blog_enqueue_css', 10 );
if( !function_exists( 'mezan_blog_enqueue_css' ) ) {
	function mezan_blog_enqueue_css() {
		wp_enqueue_style( 'wdt-blog', get_theme_file_uri('/modules/blog/assets/css/blog.css'), false, MEZAN_THEME_VERSION, 'all');
		...
		wp_enqueue_style( 'jquery-bxslider', get_theme_file_uri('/modules/blog/assets/css/jquery.bxslider.css'), false, MEZAN_THEME_VERSION, 'all' );
	}
}

add_action( 'mezan_before_enqueue_js', 'mezan_blog_enqueue_js' );
if( !function_exists( 'mezan_blog_enqueue_js' ) ) {
	function mezan_blog_enqueue_js() {
...
if( !function_exists( 'after_blog_post_content_pagination' ) ) {
    function after_blog_post_content_pagination() {
    	...
    	$pagination_template = mezan_archive_blog_post_params();
    	$pagination_template = $pagination_template['archive_blog_pagination'];

        echo apply_filters( 'mezan_blog_archive_pagination', mezan_get_template_part( 'pagination', 'templates/'.$pagination_template ) );
    }
    add_action( 'mezan_after_blog_post_content_wrap', 'after_blog_post_content_pagination' );
}

if( !function_exists( 'mezan_excerpt' ) ) {
	function mezan_excerpt( $limit = NULL ) {
